...
import (
	...
	helloworld "github.com/yukithm/grpc-example/helloworld"
	hwmock "github.com/yukithm/grpc-example/mock_helloworld"
...
func TestSayHelloAgain(t *testing.T) {
	...
	mockGreeterClient := hwmock.NewMockGreeterClient(ctrl)
	req := &helloworld.HelloRequest{Name: "unit_test"}
	mockGreeterClient.EXPECT().SayHelloAgain(
		gomock.Any(),
		&rpcMsg{msg: req},
	).Return(&helloworld.HelloReply{Message: "Mocked Interface Again"}, nil)
	testSayHelloAgain(t, mockGreeterClient)
}

func testSayHelloAgain(t *testing.T, client helloworld.GreeterClient) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := client.SayHelloAgain(ctx, &helloworld.HelloRequest{Name: "unit_test"})
	if err != nil || r.Message != "Mocked Interface Again" {
		t.Errorf("mocking failed")
	}
	t.Log("Reply : ", r.Message)
}
